...
app.post('/register', async (req, res) => {
  ...
  try {
   ...
   if (error.code === 11000) {
     return res, json({ status: "error", error: "This emailid already exists." })
    }
    throw error
  }
});
...
app.post('/adminregister', async (req, res) => {
  ...
  try {
    if (key === "root") {
      ...
      return res.json({ status: "error", error: "Enter a valid key." })
    }
  } catch (error) {
    if (error.code === 11000) {
      return res.json({ status: "error", error: "This emailidalready exists." })
    }
    throw error
  }
});
